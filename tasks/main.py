...
# 徽章行锚定“点击 y + 固定偏移”：1.12.5.29 更新前约 +40~80，更新后约 +70~94，取并集并留余量。
SEED_POPUP_NUMBER_REGION_Y_OFFSET_TOP = 35
SEED_POPUP_NUMBER_REGION_Y_OFFSET_BOTTOM = 115
# 主界面等级 OCR 顶部带高度（像素，平台无关）。
LEVEL_OCR_TOP_BAND_HEIGHT = 140
# 固定排除作物模板（始终生效）。
ALWAYS_SKIP_SEED_BUTTONS = [
    SEED_BTN_HEART_FRUIT,
    SEED_BTN_HAHA_PUMPKIN,
    SEED_BTN_COLORED_GLAZE_LOTUS,
    SEED_BTN_STARWHISPER_FLOWER,
]
    ...
    def run(self, rect: tuple[int, int, int, int]) -> TaskResult:
        ...
        if features.auto_plant:
            if self._is_degraded():
                logger.info('自动播种: 实例处于降级运行状态，跳过播种')
            else:
